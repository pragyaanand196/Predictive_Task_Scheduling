...
app.get("/api/predict", async (req, res) => {
  try {
    ...
    const scoredEmployees = employeeStats.map(stat => {
      ...
      // Speed Score: lower ratio (faster completion) is better. Let's map it.
    ...
    // Confidence percentage: compositeScore runs from 0.4 to 1.0. Let's map it nicely.
...
app.get("/api/dashboard", async (req, res) => {
  try {
    ...
    const employeeMetrics = employees.map(emp => {
      ...
      // Performance Score: combines high completion rates and speed efficiency (speed ratio < 1.0 is better)
    ...
    const activeEmployees = employeeMetrics.filter(e => e.completedCount > 0 || e.employee === "Alice Vance"); // ensure a baseline
...
// Vite Middleware integrated for dev vs. prod asset serving
